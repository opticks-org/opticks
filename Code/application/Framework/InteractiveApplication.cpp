...
int InteractiveApplication::run(int argc, char** argv)
{
   ...
   { // scope the lifetime of the lock
      ...
      for (vector<PlugInBranding>::const_iterator brandingIter = branding.begin(); brandingIter != branding.end(); ++brandingIter)
      {
         ...
         if (pFilename != NULL)
         {
            ...
            {
               splashPaths.push_back(fullPathAndName);
            }
         }
      }
      ...
      {
         // rescan the plug-ins
         ConfigurationSettingsExt2* pSettings2 = dynamic_cast<ConfigurationSettingsExt2*>(Service<ConfigurationSettings>().get());
         VERIFYRV(pSettings2 != NULL, -1);
         PlugInManagerServicesImp::instance()->buildPlugInList(pSettings2->getPlugInPath());
      }
