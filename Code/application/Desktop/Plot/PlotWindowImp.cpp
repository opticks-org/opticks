...
PlotSet* PlotWindowImp::createPlotSet(const QString& strPlotSet)
{
   ...
   if (pMenu != NULL)
   {
      string actionName = pPlotSet->getDisplayName();
      if (actionName.empty() == true)
      {
         actionName = pPlotSet->getName();
      }
...
bool PlotWindowImp::setCurrentPlotSet(PlotSet* pPlotSet)
{
   ...
   // Set the new title
   string plotSetName = pPlotSet->getDisplayName();
   if (plotSetName.empty() == true)
   {
      plotSetName = pPlotSet->getName();
   }
