...
{
   glTexImage2D(GL_TEXTURE_2D, 0, 4, sourceWidth, sourceHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE, pTexData1);

   unsigned int* pTexData2 = new unsigned int[sourceWidth * sourceHeight / 4];
   unsigned int* pDest = pTexData2;
   unsigned int* pSource = pTexData1;

   for (int k = 1; sourceWidth > 1 && sourceHeight > 1; k++)
   {
      for (int j = 0; j < sourceHeight; j += 2)
      {
         for (int l = 0; l < sourceWidth; l += 2)
         {
            unsigned int red = 0;
      ...
      glTexImage2D(GL_TEXTURE_2D, k, 4, sourceWidth / 2, sourceHeight / 2, 0, GL_RGBA, GL_UNSIGNED_BYTE, pDest);
