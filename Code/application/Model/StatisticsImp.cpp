...
void StatisticsThread::run()
{
   ...
   bool hasOneBadValue = false;
   double badValueLower = 0.0;
   double badValueUpper = 0.0;
   if (hasBadValues)
   {
      hasOneBadValue = mInput.mpBadValues->getSingleBadValueRange(badValueLower, badValueUpper);
   }
...
void HistogramThread::run()
{
      ...
      bandIt != mInput.mStatInput.mBandsToCalculate.end(); ++bandIt)
   {
      ...
      if (hasBadValues)
      {
         hasOneBadValue = mInput.mStatInput.mpBadValues->getSingleBadValueRange(badValueLower, badValueUpper);
      }
