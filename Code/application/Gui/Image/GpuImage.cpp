...
private:
      ...
      EncodingType outputType)
   {
      ...
      if (bBadValues)
      {
         ...
         bool bSingleBadValue = mInfo.mKey.mpBadValues1->getSingleBadValueRange(singleRangeLow, singleRangeHigh);
         for (unsigned int y1 = 0; y1 < tileSizeY; y1++, pTargetBase += (mInfo.mTileSizeX * totalChannels))
         {
            ...
            for (unsigned int x1 = 0; x1 < tileSizeX; x1++)
            {
               ...
               *pTarget = static_cast<Out>(getFromSource(source));
               ...
               pTarget += channelStep;
               bool bIsBadValue = false;
               double dSource = static_cast<double>(source);
               if (bSingleBadValue == true)
               {
                  bIsBadValue = dSource > singleRangeLow && dSource < singleRangeHigh;
               }
               else
               {
                  bIsBadValue = mInfo.mKey.mpBadValues1->isBadValue(dSource);
               }
   ...
   void createRgb(EncodingType outputType)
   {
      ...
      bool bSingleRedBadValue = false;
      if (hasRedBadValues)
      {
         bSingleRedBadValue = mInfo.mKey.mpBadValues1->getSingleBadValueRange(singleRangeLowRed, singleRangeHighRed);
      }
      ...
      bool bSingleGreenBadValue = false;
      if (hasGreenBadValues)
      {
         bSingleGreenBadValue = mInfo.mKey.mpBadValues2->getSingleBadValueRange(singleRangeLowGreen, singleRangeHighGreen);
      }
      ...
      bool bSingleBlueBadValue = false;
      if (hasBlueBadValues)
      {
         bSingleBlueBadValue = mInfo.mKey.mpBadValues3->getSingleBadValueRange(singleRangeLowBlue, singleRangeHighBlue);
      }
