...
unsigned int GpuImage::readTiles(double xCoord, double yCoord, GLsizei width, GLsizei height, vector<float>& values)
{
   ...
   if (destChannels == 1 /* GL_LUMINANCE */ || 
      destChannels == 3 /* GL_RGB */) // incoming data has no alpha
   {
      ++destChannels; // add space for alpha
   }

   if (values.size() < width * height * destChannels)
   {
      values.resize(width * height * destChannels);
   }
   ...
   // If there are greater than 2 tiles, it is hard to test for the need to
   ...
   {
      // If there is exactly 1 tile, we will not need to reorder the data, 
      // however tileLocations will have 1 element so front and back will
      // return the same LocationType, resulting in the dataVector being
      // allocated unnecessarily.
      // If there are 2 tiles, they may be above or along side each other.
      // If they are above, we don't need to reorder. If they are side-by-
      // side, we do need to reorder.
      if (numTiles > 1 && tileLocations.front().mY == tileLocations.back().mY)
      {
         dataVector.resize(width * height * numChannels);
      }
   }

   // If dataVector is not empty, we need to load into it and then reorder the
   // data later. Otherwise, we can load directly into values.
   ...
   float alpha = static_cast<float>(getAlpha()) / 255.0f;

   // Reorder the data into the destination vector to be row order
   // Also, add the alpha if missing
   if (dataVector.empty() == true) // data did not need reordering
   {
      switch (numChannels)
      {
         case 1: // GL_LUMINANCE: need to interleave alpha
         {
         ...
         case 3: // GL_RGB: need to interleave alpha
         {
            for (int i=values.size()-1; i>0; i-=4)
            {
               values[i] = alpha;
               values[i-1] = values[3*(i+1)/4-1];
               values[i-2] = values[3*(i+1)/4-2];
               values[i-3] = values[3*(i+1)/4-3];
            }
            break;
         }
         default: // do not need to add alpha
            break;
      }
   }
   else
      ...
      for (tileNum = 0; tileNum < numTiles; tileNum++)
      {
         for (int row = 0; row < tileHeights[tileNum]; ++row)
         {
            switch (numChannels)
            {
               case 1: // GL_LUMINANCE
               {
                  for (int i=0; i<tileWidths[tileNum]/static_cast<int>(numChannels); ++i)
                  {
                     pValueData[2*i+destinationOffsets[tileNum]] = 
                        dataVector[i+sourceOffsets[tileNum]];
                     pValueData[1+2*i+destinationOffsets[tileNum]] = alpha;
                  }
                  break;
               }
               case 3: // GL_RGB
               {
                  int destChannels = numChannels+1;
                  for (int i=0; i<tileWidths[tileNum]/static_cast<int>(numChannels); ++i)
                  {
                     pValueData[destChannels*i+destinationOffsets[tileNum]] = 
                        dataVector[numChannels*i+sourceOffsets[tileNum]];
                     pValueData[1+destChannels*i+destinationOffsets[tileNum]] = 
                        dataVector[1+numChannels*i+sourceOffsets[tileNum]];
                     pValueData[2+destChannels*i+destinationOffsets[tileNum]] = 
                        dataVector[2+numChannels*i+sourceOffsets[tileNum]];
                     pValueData[3+destChannels*i+destinationOffsets[tileNum]] = alpha;
                  }
                  break;
               }
