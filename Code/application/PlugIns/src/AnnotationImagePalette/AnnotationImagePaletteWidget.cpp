...
void AnnotationImagePaletteWidget::windowAdded(Subject& subject, const std::string& signal, const boost::any& v)
{
   SpatialDataWindow* pWindow = dynamic_cast<SpatialDataWindow*>(boost::any_cast<Window*>(v));
   if (pWindow != NULL)
   {
      pWindow->getWidget()->installEventFilter(this);
      pWindow->getWidget()->setAcceptDrops(true);
...
void AnnotationImagePaletteWidget::windowRemoved(Subject& subject, const std::string& signal, const boost::any& v)
{
   SpatialDataWindow* pWindow = dynamic_cast<SpatialDataWindow*>(boost::any_cast<Window*>(v));
   if (pWindow != NULL)
   {
   ...
   mWindows.erase(boost::any_cast<Window*>(v));
}
...
void AnnotationImagePaletteWidget::windowDropEvent(SpatialDataView* pView, QDropEvent* pEvent)
{
   ...
   sz.mX = abs(sz.mX);
   sz.mY = abs(sz.mY);
   pObject->setBoundingBox(corner, corner + sz);
   pView->refresh();
   pEvent->accept();
}