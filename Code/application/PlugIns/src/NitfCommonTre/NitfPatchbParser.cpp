   ...
   string &errorMessage) const
{
   ...
   readField<int>(input, output, success, PATCHB::LAST_PAT_FLAG, 1, errorMessage, buf);
...
Nitf::TreState Nitf::PatchbParser::isTreValid(const DynamicObject& tre, ostream& reporter) const
{
   ...
   status = MaxState(status, testTagValueRange<int>(tre, reporter, &numFields, PATCHB::LAST_PAT_FLAG, 0, 1));
   ...
   status = MaxState(status, testTagValueRange<int>(tre, reporter, &numFields, PATCHB::NVL, 20, 99999));
   status = MaxState(status, testTagValueRange<int>(tre, reporter, &numFields, PATCHB::FVL, 1, 681));
   ...
   status = MaxState(status, testTagValueRange<int>(tre, reporter, &numFields, PATCHB::FRAME, 1, 512));
   ...
   status = MaxState(status, testTagValueRange<double>(tre, reporter, &numFields, PATCHB::GRAVITY, 31.0, 33.9999));
   ...
   status = MaxState(status, testTagValueRange<double>(tre, reporter, &numFields, PATCHB::OFFLAT, -80.0, 80.0));
   status = MaxState(status, testTagValueRange<double>(tre, reporter, &numFields, PATCHB::OFFLONG, -80.0, 80.0));
   ...
   status = MaxState(status, testTagValueRange<double>(tre, reporter, &numFields, PATCHB::SHEAR, 0.85, 1.0));
   status = MaxState(status, testTagValueRange<int>(tre, reporter, &numFields, PATCHB::BATCH_NO, 1, 999999));
