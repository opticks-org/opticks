...
void ConvolutionMatrixWidget::saveToConfigurationSettings() const
{
   ...
   DynamicObject* pPresetsDo = const_cast<DynamicObject*>(var.getPointerToValue<DynamicObject>());
   if (pPresetsDo == NULL)
   {
      pPresetsDo = reinterpret_cast<DynamicObject*>(
         Service<ObjectFactory>()->createObject(TypeConverter::toString<DynamicObject>()));
      Service<ConfigurationSettings>()->setSetting("ConvolutionMatrixEditor/Presets",
         DataVariant(TypeConverter::toString<DynamicObject>(), pPresetsDo, false));
   }
   pPresetsDo->clear();
