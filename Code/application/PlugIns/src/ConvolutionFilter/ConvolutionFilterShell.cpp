...
bool ConvolutionFilterShell::displayResult()
{
   if (mInput.mpResult == NULL)
   {
      return false;
...
void ConvolutionFilterShell::ConvolutionFilterThread::convolve(const T*)
{
   ...
   int numBands = mInput.mpDescriptor->getBandCount();
   int numRows = mRowRange.mLast - mRowRange.mFirst + 1;
   int numResultsCols = 0;

   if (mInput.mpCheck->useAllPixels())
   {
      numResultsCols = numCols;
   }
   else
   {
      numResultsCols = mInput.mpCheck->getNumColumns();
   }
