...
{
   // Assign a default name for the feature class based on the number of existing classes
   QString className = "Feature Class " + QString::number(mpClassList->count() + 1);
...
{
   // Assign a default name for the field based on the number of existing fields
   QString fieldName = "Field " + QString::number(mpFieldTree->topLevelItemCount() + 1);
   ...
   mpFieldTree->clearSelection();   // The QTreeWidget does not automatically deselect any selected items
                                    // when a new item is selected
   pFieldItem->setSelected(true);
}
